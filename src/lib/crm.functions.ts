  ...
  .handler(async ({ context, data }) => {
    try {
      ...
      let query = supabaseAdmin
        .from("threads")
        .select("id, contact_id, last_message_at, unread_count, assigned_to_user_id, ai_enabled, purchase_intent, contacts:contact_id(id, display_name, wa_id, phone, profile_picture_url)")
        .eq("org_id", orgId);
