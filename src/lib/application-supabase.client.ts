...
function createSupabaseClient() {
  ...
  const envPresence = {
    ...
    SUPABASE_URL: Boolean(typeof process !== 'undefined' ? process.env.SUPABASE_URL : undefined),
    SUPABASE_PUBLISHABLE_KEY: Boolean(typeof process !== 'undefined' ? process.env.SUPABASE_PUBLISHABLE_KEY : undefined),
  };
  ...
  if (!SUPABASE_URL || !SUPABASE_PUBLISHABLE_KEY) {
    ...
    console.error(`[Supabase] ${message}`, envPresence);
    throw new Error(`${message} Env presence: ${JSON.stringify(envPresence)}`);
  }
