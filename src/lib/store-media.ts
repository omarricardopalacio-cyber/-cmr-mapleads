...
export function isDirectPlayableVideo(url: string | null | undefined): boolean {
  ...
  if (youtubeEmbedUrl(u) || vimeoEmbedUrl(u)) return false;
  if (/drive\.google|docs\.google|dropbox\.com\/s\/|facebook\.com|tiktok\.com|instagram\.com/i.test(u)) {
    return false;
  }
  // Extensión de video o storage/CDN conocidos
  ...
  // Sin extensión: muchos catálogos Syncro usan signed URLs de storage → intentar
...
/** Prioriza video (embed o directo); si no, imagen. */
export function resolveStoreMedia(
  ...
  if (v) {
    ...
    if (isDirectPlayableVideo(v)) return { kind: "video", src: v };
    // Video no reproducible inline → imagen a pantalla completa + el link se maneja aparte
